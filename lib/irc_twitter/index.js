...
function irc_twitter() {
    ...
    self.connect = function (channel) {
        if (!app.config.twitter.channels[channel].twitter) {
            return;
        }

        self.cons.connecting.push(channel);
        self.getTwitterId(app.config.twitter.channels[channel].twitter, channel, self.streamStart);
    };
    ...
    self.streamRestart = function (channel, method, response, status) {
        ...
        if (connecting_id == -1) {
            if (method === 'end' && status == 420) {
                setTimeout(function () {
                    self.log('Waiting 60 seconds before connecting again.. twitter is rate limiting us.');
