var util = require('util'),
    ntwitter = new (require('ntwitter'))(app.config.twitter),
...
function irc_twitter() {
    ...
    self.accounts = {};
    self.channels = {};
    self.verbose = app.config.twitter_verbose || false;
    ...
    self.getTwitterId = function (account, channel, cb) {
        ...
        for (var account_id in self.accounts) {
            if (self.accounts[account_id].account == account) {
                self.log('Found (existing) id for ' + account + ' (' + account_id + ')');
        ...
        ntwitter.showUser(account, function (err, data) {
            ...
            if (err) {
                ...
                self.log('Found id for ' + account + ' (' + account_id + ')');
    ...
    self.streamOnData = function (data) {
        ...
        self.log('tweet recieved (' + channels.join(', ') + '): ' + message); // clean the message so it doesnt have colour codes, bold, etc?
        ...
        for (var i = 0, l = channels.length; i < l; i++) {
            ...
            for (var blocked_user in self.channels[channel].users) {
                ...
                if (data.retweeted_status !== undefined && re.test(data.retweeted_status.user.screen_name)) {
                    self.log('Retweet blocked (' + channel + '), by ' + blocked_user);
            ...
            for (var blocked_content in self.channels[channel].content) {
                ...
                if (data.retweeted_status !== undefined) {
                    ...
                    if (re.test(data.retweeted_status.text)) {
                        self.log('Retweet blocked (' + channel + '), contained "' + blocked_content + '"');
                        continue; // move to next channel, it might not be blocked there!
                    }
                } else {
                    // check normal
                    if (re.test(data.text)) {
                        self.log('Tweet blocked (' + channel + '), contained "' + blocked_content + '"');
                        continue; // move to next channel, it might not be blocked there!
                    }
                }
            }
    ...
    self.streamRestart = function (method, response, status) {
        self.log(method + ' > ' + response + (status !== undefined ? ' (' + status + ')' : ''));
